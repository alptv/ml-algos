from boost import AdaptiveBoost
import numpy as np
import pandas as pd
from read import accuracy
...
def read(data):
    targets = np.array(data['class'].map(lambda c: 1 if c == 'P' else 0))
    data = data.drop('class', 1)
    return data.to_numpy(), targets


name = 'geyser'
file = '/home/alexander/Projects/Python/dt/boost-data/' + name + '.csv'
data = pd.read_csv(file)
dataset, targets = read(data)
steps = [1, 2, 3, 5, 8, 13, 21, 34, 55]
for i in steps:
    print(i)
    adaboost = AdaptiveBoost(dataset, targets, i)
    adaboost.train()
    draw_graphic(dataset, targets, adaboost, name + ' ' + str(i), name + ' ' + str(i))
